# Aether Board // Engineering Infinite Canvas

[Português] | [English]

---

## 🇧🇷 Português

O **Aether Board** é um quadro branco infinito focado em engenharia e produtividade científica, permitindo o desenho livre e a renderização instantânea de fórmulas **LaTeX**.

### Como Usar Agora
Se você está no Windows, não precisa instalar nada! 
1. Execute o arquivo `AetherBoard_PRO.exe` que está na raiz do projeto.

### Funcionalidades
- **Canvas Infinito**: Espaço ilimitado para suas ideias.
- **Renderização LaTeX**: Digite fórmulas matemáticas e veja-as aparecerem instantaneamente.
- **Auto-Save**: Seus desenhos são salvos automaticamente ao fechar.
- **Estética premium**: Interface inspirada no estilo Frutiger Aero.

---

## 🇺🇸 English

**Aether Board** is an infinite whiteboard designed for engineering and scientific productivity, featuring free-hand drawing and instant **LaTeX** formula rendering.

### Quick Start
If you are on Windows, no installation is required!
1. Run the `AetherBoard_PRO.exe` file located in the project root.

### Features
- **Infinite Canvas**: Unlimited space for your brainstorms.
- **LaTeX Rendering**: Type math formulas and see them appear instantly.
- **Auto-Save**: Your progress is automatically saved upon closing.
- **Premium Aesthetics**: Interface inspired by the Frutiger Aero style.

---

